 ...
 * Resolves to { ok, error, me, roster, logs, overrides }.
 * Never throws.
 */
export async function loadAll(me) {
  const c = getClient();
  if (!c || !me) return { ok: false, error: "Not signed in.", roster: [], logs: {}, overrides: {} };

  try {
    const [links, profiles, logs, overrides] = await Promise.all([
    ...
    const firstError = links.error || profiles.error || logs.error || overrides.error;
    if (firstError) return { ok: false, error: firstError.message, roster: [], logs: {}, overrides: {} };
    ...
    return { ok: false, error: "Could not reach the server.", roster: [], logs: {}, overrides: {} };
  }
}
